package com.robot.platform.framework.websocket.config;

import com.robot.platform.framework.mq.redis.config.RobotPlatformRedisMQConsumerAutoConfiguration;
import com.robot.platform.framework.mq.redis.core.RedisMQTemplate;
import com.robot.platform.framework.websocket.core.handler.JsonWebSocketMessageHandler;
import com.robot.platform.framework.websocket.core.listener.WebSocketMessageListener;
import com.robot.platform.framework.websocket.core.security.LoginUserHandshakeInterceptor;
import com.robot.platform.framework.websocket.core.security.WebSocketAuthorizeRequestsCustomizer;
import com.robot.platform.framework.websocket.core.sender.kafka.KafkaWebSocketMessageConsumer;
import com.robot.platform.framework.websocket.core.sender.kafka.KafkaWebSocketMessageSender;
import com.robot.platform.framework.websocket.core.sender.local.LocalWebSocketMessageSender;
import com.robot.platform.framework.websocket.core.sender.rabbitmq.RabbitMQWebSocketMessageConsumer;
import com.robot.platform.framework.websocket.core.sender.rabbitmq.RabbitMQWebSocketMessageSender;
...
import com.robot.platform.framework.websocket.core.sender.rocketmq.RocketMQWebSocketMessageConsumer;
import com.robot.platform.framework.websocket.core.sender.rocketmq.RocketMQWebSocketMessageSender;
import com.robot.platform.framework.websocket.core.session.WebSocketSessionHandlerDecorator;
import com.robot.platform.framework.websocket.core.session.WebSocketSessionManager;
 ...
 * @author xingyu4j
 */
@AutoConfiguration(before = RobotPlatformRedisMQConsumerAutoConfiguration.class) // before RobotPlatformRedisMQConsumerAutoConfiguration 的原因是，需要保证 RedisWebSocketMessageConsumer 先创建，才能创建 RedisMessageListenerContainer
@EnableWebSocket // 开启 websocket
@ConditionalOnProperty(prefix = "robot-platform.websocket", value = "enable", matchIfMissing = true) // 允许使用 robot-platform.websocket.enable=false 禁用 websocket
@EnableConfigurationProperties(WebSocketProperties.class)
public class RobotPlatformWebSocketAutoConfiguration {
    ...
    @Configuration
    @ConditionalOnProperty(prefix = "robot-platform.websocket", name = "sender-type", havingValue = "local")
    ...
    @Configuration
    @ConditionalOnProperty(prefix = "robot-platform.websocket", name = "sender-type", havingValue = "redis")
    ...
    @Configuration
    @ConditionalOnProperty(prefix = "robot-platform.websocket", name = "sender-type", havingValue = "rocketmq")
    public class RocketMQWebSocketMessageSenderConfiguration {
        ...
        public RocketMQWebSocketMessageSender rocketMQWebSocketMessageSender(
                ...
                @Value("${robot-platform.websocket.sender-rocketmq.topic}") String topic) {
    ...
    @Configuration
    @ConditionalOnProperty(prefix = "robot-platform.websocket", name = "sender-type", havingValue = "rabbitmq")
    public class RabbitMQWebSocketMessageSenderConfiguration {
         ...
         */
        @Bean
        public TopicExchange websocketTopicExchange(@Value("${robot-platform.websocket.sender-rabbitmq.exchange}") String exchange) {
    ...
    @Configuration
    @ConditionalOnProperty(prefix = "robot-platform.websocket", name = "sender-type", havingValue = "kafka")
    public class KafkaWebSocketMessageSenderConfiguration {
        ...
        public KafkaWebSocketMessageSender kafkaWebSocketMessageSender(
                ...
                @Value("${robot-platform.websocket.sender-kafka.topic}") String topic) {
