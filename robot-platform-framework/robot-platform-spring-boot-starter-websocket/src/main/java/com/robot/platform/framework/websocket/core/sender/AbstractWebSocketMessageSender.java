package com.robot.platform.framework.websocket.core.sender;

import cn.hutool.core.collection.CollUtil;
import cn.hutool.core.util.StrUtil;
import com.robot.platform.framework.common.util.json.JsonUtils;
import com.robot.platform.framework.websocket.core.message.JsonWebSocketMessage;
...
public abstract class AbstractWebSocketMessageSender implements WebSocketMessageSender {
    ...
    public void send(String sessionId, Integer userType, Long userId, String messageType, String messageContent) {
        ...
        if (tenantId == null) throw new IllegalStateException("WebSocket tenant is required");
        // 1. 获得 Session 列表
        ...
        // Defense in depth: a session lookup must never defeat tenant routing.
        doSend(sessions.stream().filter(session -> tenantId.equals(WebSocketFrameworkUtils.getTenantId(session))).toList(), messageType, messageContent);
    }
