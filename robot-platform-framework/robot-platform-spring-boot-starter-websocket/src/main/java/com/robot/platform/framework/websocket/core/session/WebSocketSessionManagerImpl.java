package com.robot.platform.framework.websocket.core.session;
...
public class WebSocketSessionManagerImpl implements WebSocketSessionManager {
    ...
    public Collection<WebSocketSession> getSessionList(Integer userType) {
        ...
        for (List<WebSocketSession> sessions : userSessionsMap.values()) {
            ...
            for (WebSocketSession session : sessions) {
                // The same user id can have sessions in more than one tenant after a switch.
                if (contextTenantId == null || contextTenantId.equals(WebSocketFrameworkUtils.getTenantId(session))) result.add(session);
            }
        }
        return result;
    }
    ...
    public Collection<WebSocketSession> getSessionList(Integer userType, Long userId) {
        ...
        if (CollUtil.isEmpty(sessions)) return new ArrayList<>();
        Long tenant = TenantContextHolder.getTenantId();
        return tenant == null ? new ArrayList<>(sessions) : sessions.stream()
                .filter(session -> tenant.equals(WebSocketFrameworkUtils.getTenantId(session))).toList();
    }
