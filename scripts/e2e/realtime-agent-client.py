...
async def run(a):
 ...
 async with websockets.connect(uri,additional_headers={"Authorization":"Bearer "+token}) as ws:
  started=time.monotonic();identity={"memberId":a.member_id,"identityType":"APP_BOUND","identityConfidence":1.0} if a.member_id else None
  await ws.send(evt("session.start",agentCode=a.agent_code,audio={"encoding":"PCM_S16LE","sampleRate":16000,"channels":1},identity=identity));await ws.send(evt("input.speech_started"));await ws.send(Path(a.pcm).read_bytes());await ws.send(evt("input.speech_stopped"))
  async for raw in ws:
...
def main():
 p=argparse.ArgumentParser();p.add_argument("--base",default=os.getenv("REALTIME_BASE_URL","ws://localhost:48080"));p.add_argument("--agent-code",default=os.getenv("AGENT_CODE","qwen-native"));p.add_argument("--pcm",default=str(Path(__file__).with_name("fixtures")/"silence-16k-s16le.pcm"));p.add_argument("--member-id",type=int);p.add_argument("--interrupt",action="store_true");a=p.parse_args()
