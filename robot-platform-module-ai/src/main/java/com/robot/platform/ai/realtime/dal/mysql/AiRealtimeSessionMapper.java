...
public interface AiRealtimeSessionMapper extends BaseMapperX<AiRealtimeSessionDO> {
    ...
    @TenantIgnore
    @Select("SELECT * FROM ai_realtime_session WHERE tenant_id = #{tenantId} ORDER BY created_at DESC")
    List<AiRealtimeSessionDO> selectAllByTenantId(@Param("tenantId") long tenantId);
}
