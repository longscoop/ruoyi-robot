...
依赖：docker compose v2。若必须使用旧版 `docker-compose`，请从 compose 文件中删除 `name: robot-platform-system`，并将 `version` 降低到 `3.3` 以下。
...
```shell
# 创建maven缓存volume
docker volume create --name robot-platform-maven-repo

docker run -it --rm --name robot-platform-maven \
    -v robot-platform-maven-repo:/root/.m2 \
