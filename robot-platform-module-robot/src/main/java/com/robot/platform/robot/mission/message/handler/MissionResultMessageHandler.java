...
@Service
public class MissionResultMessageHandler extends AbstractMissionMessageHandler<MissionEventPayload> {
    public MissionResultMessageHandler(RobotMessageInboxService inbox, MissionService missions) { super(inbox, missions); }
    ...
    public MessageHandleResult handle(DeviceIdentity device, RobotMessageEnvelope<MissionEventPayload> envelope) {
        return once(device, envelope, "MISSION_EVENT", (identity, message) -> {
            if (!"RESULT".equals(message.data().kind())) throw new IllegalArgumentException("mission event is not a result");
            return missions.handleResult(identity.deviceId(), message.requestId(), message.messageId(), message.data());
        });
    }
}
