package com.robot.platform.module.infra.service.codegen.inner;
...
import com.robot.platform.framework.apilog.core.enums.OperateTypeEnum;
...
import com.robot.platform.framework.common.util.collection.CollectionUtils;
import com.robot.platform.framework.common.util.date.DateUtils;
import com.robot.platform.framework.common.util.date.LocalDateTimeUtils;
import com.robot.platform.framework.common.util.object.BeanUtils;
import com.robot.platform.framework.common.util.object.ObjectUtils;
import com.robot.platform.framework.common.util.string.StrUtils;
import com.robot.platform.framework.excel.core.annotations.DictFormat;
import com.robot.platform.framework.excel.core.convert.DictConvert;
import com.robot.platform.framework.excel.core.util.ExcelUtils;
...
import com.robot.platform.module.infra.dal.dataobject.codegen.CodegenColumnDO;
import com.robot.platform.module.infra.dal.dataobject.codegen.CodegenTableDO;
import com.robot.platform.module.infra.enums.codegen.CodegenFrontTypeEnum;
import com.robot.platform.module.infra.enums.codegen.CodegenSceneEnum;
import com.robot.platform.module.infra.enums.codegen.CodegenTemplateTypeEnum;
import com.robot.platform.module.infra.enums.codegen.CodegenVOTypeEnum;
import com.robot.platform.module.infra.framework.codegen.config.CodegenProperties;
...
import static com.robot.platform.module.infra.enums.ErrorCodeConstants.CODEGEN_MASTER_TABLE_FIELD_DUPLICATE;
import static com.robot.platform.module.infra.enums.ErrorCodeConstants.CODEGEN_MASTER_TABLE_NAME_DUPLICATE;
...
public class CodegenEngine {
    ...
    /**
     * 是否为 robot-platform-cloud 项目，用于解决 Boot 和 Cloud 的 api 模块兼容性问题
     *
     * true  - 需要有 robot-platform-module-xxx-api 模块
    ...
    public CodegenEngine() {
        ...
        this.cloudEnable = ClassUtils.isPresent("com.robot.platform.module.infra.framework.rpc.config.RpcConfiguration",
    ...
    private Map<String, String> getTemplates(Integer frontType) {
        ...
        if (Boolean.FALSE.equals(cloudEnable)) {
            SERVER_TEMPLATES.forEach((templatePath, filePath) -> {
                filePath = StrUtil.replace(filePath, "/robot-platform-module-${table.moduleName}-api", "");
                filePath = StrUtil.replace(filePath, "/robot-platform-module-${table.moduleName}-server", "");
    ...
    private static String javaModuleFilePath(String path, String module, String src) {
        return "robot-platform-module-${table.moduleName}/" + // 顶级模块
                "robot-platform-module-${table.moduleName}-" + module + "/" + // 子模块
                "src/" + src + "/java/${basePackage}/module/${table.moduleName}/" + path + ".java";
    }

    private static String mapperXmlFilePath() {
        return "robot-platform-module-${table.moduleName}/" + // 顶级模块
                "robot-platform-module-${table.moduleName}-server/" + // 子模块
    ...
    private static String vueFilePath(String path) {
        return "robot-platform-ui-${sceneEnum.basePackage}-vue2/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3FilePath(String path) {
        return "robot-platform-ui-${sceneEnum.basePackage}-vue3/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3UniappFilePath(String path) {
        return "robot-platform-ui-${sceneEnum.basePackage}-uniapp/" + // 顶级目录
                "src/" + path;
    }

    private static String vue3VbenFilePath(String path) {
        return "robot-platform-ui-${sceneEnum.basePackage}-vben/" + // 顶级目录
                "src/" + path;
    }
